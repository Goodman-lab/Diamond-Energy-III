...
from rdkit.Chem import rdmolops  # Import this for removing stereochemistry


# Load the data
data = pd.read_pickle('qm9_generated.pkl')

# Create an empty dictionary for storing SMILES strings
test_set_smiles = {}

# Convert InChI to SMILES without stereochemistry
for name, inchi in test_set_inchi.items():
    mol = Chem.MolFromInchi(inchi)
    if mol is not None:
        smiles = Chem.MolToSmiles(mol, isomericSmiles=False)
        test_set_smiles[name] = smiles

print(test_set_smiles)

# Create a new dictionary mapping SMILES to names
smiles_to_name = {v: k for k, v in test_set_smiles.items()}

# Group molecules by their SMILES strings
mol_dict = {}
for mol in data:
    smiles = Chem.MolToSmiles(mol, isomericSmiles=False)
...
# Process each group of conformers based on their SMILES
for smiles, mols in mol_dict.items():
    mol_name = smiles_to_name.get(smiles, f"unknown_{smiles}")
    ...
    for i, mol in enumerate(mols):
        ...
        # Initialize SDWriter to output a single conformer to its own SDF file
        output_filename = f"{mol_name}_{i + 1}_CGCFoutput.sdf"
        writer = SDWriter(output_filename)

        # Write to the SDF file
        writer.write(mol_h)
...
print("SDF files have been created.")
